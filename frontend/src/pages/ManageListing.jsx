...
const ManageListing = () => {
  ...
  const fuel_type = ["Petrol", "Diesel", "Electric"];
        ...
        <form onSubmit={handleSubmit} className='space-y-8'>
          ...
          <Section title="Vehicle Details">
            <div className='grid grid-cols-1 md:grid-cols-3 gap-6 mb-6'>
              ...
              <InputField label='Interior Color' value={formData.interior_color_color} placeholder='e.g., Maroon' onChange={(v) => handleInputChange('interior_color', v)} required={false} />
              ...
              <InputField label='Engine Capacity (cc)' type='number' min={0} value={formData.engine_capacity_cc} placeholder='6' onChange={(v) => handleInputChange('engine_capacity_cc', v)} required={false} />
              <InputField label='Mileage (km) *' value={formData.mileage_km} placeholder='6' onChange={(v) => handleInputChange('mileage_km', v)} required={true} />
            </div>
          </Section>

          {/* PRICING */}
          <Section title='Pricing and Description'>
            <InputField label='Rental Price (LKR) /day *' type='number' min={0} value={formData.price_per_day} placeholder='9500.00' onChange={(v) => handleInputChange('price', v)} required={true} />
          ...
          <Section title='Vehicle Images'>
            <div className='border-2 border-dashed border-gray-300 rounded-lg p-6 text-center'>
              ...
              <p className='text-sm text-gray-500 mt-2'> Upload Images of the Vehicle </p>
            </div>
...
const InputField = ({label, value, onChange, placeholder, type = 'text', required = false, min = null, max = null})=> (
  <div>
    <label className='block text-sm font-medium text-gray-700' mb-2>{label}</label>
      <input type={type} min={min} max={max} placeholder={placeholder} value={value} onChange={(e)=>onChange(e.target.value)} className='w-full px-3 py-1.5
      text-gray-600 border rounded-md focus:outline-none focus:ring-2 focus:ring-red-500 border-gray-300' required={required}/>
  </div>
)
...
const SelectField = ({label, options, value, onChange, required = false}) => (
  <div>
    <label className='block text-sm font-medium text-gray-700' mb-2>{label}</label>
    <select value={value} onChange={(e) => onChange(e.target.value)} className='w-full px-3 py-1.5 text-gray-600 border rounded-md focus:outline-none 
    focus:ring-2 focus:ring-red-500 border-gray-300' required={required}>
      <option value=''>
        Select...
      </option>
      {options.map((opt)=>(
...
const TextareaField = ({label, options, value, onChange, required = false}) => (
  <div>
    <label className='block text-sm font-medium text-gray-700 mb-2'>{label}</label>
    <textarea rows={5} value={value} onChange={(e) => onChange(e.target.value)} className='w-full px-3 py-1.5 text-gray-600 border rounded-md
    focus:outline-none focus:ring-2 focus:ring-red-500 border-gray-300'></textarea>
  </div>
)
